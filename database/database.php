...
class database{
    ...
    public function receptieLogin($gebruikersnaam, $wachtwoord) {
        // id, voorletters, voorvoegsels, achternaam, gebruikersnaam, wachtwoord 
        $sql ="SELECT id, gebruikersnaam, wachtwoord FROM receptie WHERE gebruikersnaam = :gebruikersnaam";
        // prepare
    ...
    public function addAndGetLastId($statement, $named_placeholder){
        $stmt = $this->conn->prepare($statement);
        $stmt->execute($named_placeholder); //['uname'=>$username]
    ...
    public function add($statement, $named_placeholder){
        $stmt = $this->conn->prepare($statement);
        $stmt->execute($named_placeholder);
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
